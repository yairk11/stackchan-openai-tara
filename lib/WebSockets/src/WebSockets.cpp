...
bool WebSockets::handleWebsocketWaitFor(WSclient_t * client, size_t size) {
    ...
    readCb(client, &client->cWsHeader[client->cWsRXsize], (size - client->cWsRXsize), std::bind([](WebSockets * server, size_t size, WSclient_t * client, bool ok) {
        ...
        if(ok) {
            ...
            client->cWsRXsize = 0;
            // timeout or error
            Serial.println("WS1002 PATH=READCB_FAIL");
    ...
    if(ok) {
        ...
        switch(header->opCode) {
            ...
            default:
                ...
                Serial.println("WS1002 PATH=UNKNOWN_OPCODE");
        ...
        free(payload);
        Serial.println("WS1002 PATH=MISSING_DATA");
        clientDisconnect(client, 1002);
    }
}
...
size_t WebSockets::write(WSclient_t * client, uint8_t * out, size_t n) {
...
#if defined(ESP32) && (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP32)
    if(!client->isSSL) {
        if(client->tcp == NULL) {
        ...
        int socketFd = client->tcp->fd();

        if(socketFd < 0) {
            DEBUG_WEBSOCKETS("[write] invalid socket fd\n");
            return 0;
        }

        static const uint32_t ESP32_WS_WRITE_TIMEOUT_MS = 2000;
        ...
        while(n > 0) {
            ...
            if((millis() - started) >= ESP32_WS_WRITE_TIMEOUT_MS) {
                Serial.printf(
                    "WS WRITE FAST-FAIL timeout=%lums left=%u\n",
            ...
            if(ready < 0) {
                ...
                Serial.printf(
                    "WS WRITE FAST-FAIL select errno=%d\n",
                    errno
                );
                break;
            }
            ...
            Serial.printf(
                "WS WRITE FAST-FAIL send=%d errno=%d\n",
                sent,
                errno
            );
            break;
    ...
    while(n > 0) {
        ...
        if((millis() - t) > WEBSOCKETS_TCP_TIMEOUT) {
            DEBUG_WEBSOCKETS(
                "[write] write TIMEOUT! %lu\n",
                (millis() - t)
            );
            break;
        }

        len = client->tcp->write(
            (const uint8_t *)out,
            n
        );
